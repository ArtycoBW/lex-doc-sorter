...
function getPlanLabel(user: ReturnType<typeof useAuth>["user"]) {
  if (!user) return "Пилот"
  if (user.role === "ADMIN") return "Admin"
  if (user.access?.currentTariff?.name) return user.access.currentTariff.name
...
function Sidebar({
  ...
  const tokenBalance = user?.tokenBalance ?? user?.access?.extraTokenBalance ?? 0
      ...
      <div className={cn("px-3 pb-3", collapsed && "flex justify-center px-2")}>
        <Button
          ...
          className={cn(
            ...
            collapsed
              ? "h-9 w-9"
              : "h-9 w-full justify-start gap-2.5 rounded-xl text-sm hover:bg-muted/60",
          )}
          onClick={onFeedback}
        >
          <MessageSquare className="h-4 w-4" />
          {!collapsed && "Обратная связь"}
        </Button>
      </div>

      <div className={cn("space-y-2 border-t border-border p-3", collapsed && "flex flex-col items-center")}>
        {collapsed ? (
          <>
            <Link href="/profile" title={displayName} onClick={onNavigate}>
            ...
            <Button
              ...
              variant="ghost"
              size="icon"
              aria-label="Тариф и токены"
              title={`${planLabel}: ${formatCompactNumber(tokenBalance)} токенов`}
          ...
          <>
            <div className="sidebar-label-reveal flex items-center gap-2 rounded-xl border border-border bg-muted/28 p-1.5">
              <Link
                href="/profile"
                onClick={onNavigate}
                className="flex min-w-0 flex-1 items-center gap-3 rounded-lg px-1.5 py-1 transition-colors hover:bg-muted/50"
              >
                <UserAvatar name={displayName} />
            ...
            <button
              type="button"
              className="sidebar-label-reveal flex w-full items-center justify-between gap-3 rounded-xl border border-border/80 bg-background/45 px-3 py-2.5 text-left transition-colors hover:bg-muted/45"
              onClick={onBilling}
            >
              <span className="flex min-w-0 items-center gap-2">
                ...
                <span className="min-w-0">
                  ...
                  <span className="mt-0.5 flex items-center gap-1 text-xs text-muted-foreground">
                    ...
                    {formatCompactNumber(tokenBalance)} токенов
                  </span>
                </span>
              </span>
              <span className="rounded-full border border-border bg-card px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">
                План
              </span>
            </button>
